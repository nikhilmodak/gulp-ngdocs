...
exports.Example.prototype.toHtmlEdit = function() {
  ...
  exports.Example.prototype.toHtmlTabs = function() {
    ...
    /**
     * Appends scripts/styles contents as templates
     * @param  {object} sources Contains a list of created sources (html, css, js, json, unit, scenario.)
     */
    function hostTag(sources) {
      sources.forEach(function(source) { 
        var isCss = source.name.match(/\.css$/);
        var tag = '';

        if(isCss) {
          tag = '<style type="text/css" id="' + source.id + '">' + source.content + '</style>\n';
        } else {
          tag = '<script type="text/ng-template" id="' + source.id + '">' + source.content + '</script>\n';
        }
